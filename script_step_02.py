        ...
        for feature in layer.getFeatures():
            ...
            if sidewalk_bicycle in ['yes', 'designated', 'permissive'] or sidewalk_both_bicycle in ['yes', 'designated', 'permissive'] or sidewalk_left_bicycle in ['yes', 'designated', 'permissive']:
                if vars_settings.offset_distance == 'realistic':
                    ...
                    offset_sidewalk_left = helper_functions.cast_to_float(vars_settings.offset_distance)
            ...
            with edit(offset_layer):
                for feature in offset_layer.getFeatures():
                    ...
                    if (
                        way_type != 'cycleway'
                        or (
                            way_type == 'cycleway' and (
                                (
                                    feature.attribute('cycleway:' + side) == 'track'
                                    or feature.attribute('cycleway:both') == 'track'
                                    or feature.attribute('cycleway') == 'track'
                                )
                                or feature.attribute(way_type + ':' + side + ':surface') != NULL
                                or feature.attribute(way_type + ':both:surface') != NULL
                                or feature.attribute(way_type + ':surface') != NULL
                            )
                        )
                    ):
                    ...
                    if (
                        ...
                        or (
                            way_type == 'cycleway' and (
                                ...
                                or feature.attribute(way_type + ':' + side + ':smoothness') != NULL
                                or feature.attribute(way_type + ':both:smoothness') != NULL
                                or feature.attribute(way_type + ':smoothness') != NULL
                            )
                        )
                    ):
